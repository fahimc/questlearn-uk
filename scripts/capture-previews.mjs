...
try{
  ...
  for(const [name,path,width,height] of jobs){const wait=path.startsWith('games/blocksmith')||path.startsWith('games/skybound')?'1.5':'0.35';await run([`http://127.0.0.1:4174/${path}`,'-o',resolve(root,`docs/images/responsive/${name}.png`),'--width',String(width),'--height',String(height),'--wait',wait]);}
