import { createInfiniteQuestionSet, validateGeneratedQuestion } from './curriculum-question-generator.js';
...
export function createWordwallWorldQuestions(worldId='english',set=0){
  const world=getWordwallWorld(worldId),questions=createInfiniteQuestionSet({subject:world.subject,set,count:WORDWALL_LEVEL_COUNT});
  return Object.freeze(questions.map((question,index)=>Object.freeze({...question,number:index+1,worldId:world.id})));
}

export function validateWordwallWorlds(worlds=WORDWALL_WORLDS){
  for(const id of WORDWALL_WORLD_IDS){const world=worlds[id];if(!world||world.id!==id||!world.subjectName||world.palette.length!==WORDWALL_LEVEL_COUNT)throw new Error(`Invalid Wordwall world ${id}`);const questions=createWordwallWorldQuestions(id,0);if(questions.length!==WORDWALL_LEVEL_COUNT||questions.some((question,index)=>question.number!==index+1||!validateGeneratedQuestion(question)))throw new Error(`Invalid question route for ${id}`);for(let index=1;index<questions.length;index+=1)if(questions[index].level<questions[index-1].level)throw new Error(`Question progression must rise in ${id}`)}return true;
}
