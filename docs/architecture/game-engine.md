...
LexiClimb reuses Skybound's camera-relative movement, analogue input, buffered jump, procedural audio and low-power selection. Its six deterministic course plans contain 48 rotated box, circle and triangle supports, seven checkpoint platforms and six collapsible word walls arranged as a one-and-a-half-turn helix around a central tower. `wordwall-engine.js` owns the spiral pose formula, shape-specific minimum clearance, shape-accurate rotated footprints, solid side/underside collision, gate collision, checkpoint/challenge progression, answer normalisation and the validated coin/skin profile. Three.js consumes the engine-sized dimensions for its meshes and owns presentation, wall animation and a single instanced batch for all 30 collectible coins.
