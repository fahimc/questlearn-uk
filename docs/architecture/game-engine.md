...
LexiClimb reuses Skybound's camera-relative movement, analogue input, buffered jump, procedural audio and low-power selection. Each of its three themed worlds has five deterministic course plans, 40 rotated box/circle/triangle supports, six checkpoint platforms, five physical level flags and five collapsible curriculum walls on one continuous helix. `curriculum-question-bank.js` materialises the 100 reviewed prototypes; the seeded generator produces reproducible five-level routes. `wordwall-engine.js` owns the spiral pose formula, shape-specific minimum clearance, shape-accurate rotated footprints, solid side/underside collision, gate collision, checkpoint/challenge progression, answer normalisation and validated coin/skin profile. Three.js consumes the engine-sized dimensions and owns presentation, flags, wall animation and one instanced batch for all 30 collectible coins.
