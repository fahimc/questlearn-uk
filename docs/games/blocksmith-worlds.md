...
The prototype has a low-power rendering path for Chromebooks, tablets and constrained devices. The deterministic terrain is split into 16 × 16 instance chunks and culled outside the active draw radius. Trees, leaves, rocks, resource piles, flowers, hedges and riverbanks are rendered in shared instance batches instead of hundreds of separate draw calls, while individual blocks remain mineable and renewable. The low tier caps device pixel ratio at 1, uses a 42-block draw distance, disables antialiasing and shadows, and targets 45 frames per second. DOM, radar, labels and chunk visibility update at a slower cadence than the 3D movement loop; rendering falls to 20 frames per second behind an open dialog and pauses in a hidden tab. A headless low-tier runtime capture reports 57 draw calls and 74,340 triangles for the initial world view. `?quality=low` and `?quality=high` provide deterministic test overrides.
