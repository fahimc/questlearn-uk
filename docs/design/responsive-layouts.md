...
- Learn and Hint open a bounded teaching panel below the compact HUD only when requested. Closing the selected action restores the unobstructed playfield without changing gameplay state.
- Touch movement and Jump are independent pointer controls so the learner can run and jump at the same time.
- Portrait uses a shallow full-width question strip below the HUD and bottom-corner controls. Short landscape uses a 310 px question card in the upper-left, smaller controls and the rest of the viewport for the bridge.
